  ...
  try {
    ...
    try {
      ...
      if (geocodingResponse.data && geocodingResponse.data.length > 0) {
        ...
        // If no results, try searching by city and country only
        const cityGeocoding = await axios.get(
        ...
        if (cityGeocoding.data && cityGeocoding.data.length > 0) {
          longitude = parseFloat(cityGeocoding.data[0].lon);
          latitude = parseFloat(cityGeocoding.data[0].lat);
        }
      }
    } catch (geocodeError) {
      console.error("Geocoding error:", geocodeError);
      // Keep default 0,0 if geocoding completely fails
    }
